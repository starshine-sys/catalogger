...
// All exported methods on Client are safe to use even when Client is nil.
type Client struct {
	Client api.WriteAPI
...
func (c *Client) submitInner() {
	...
	log.Infof("Submitting metrics to InfluxDB")
	...
	if err != nil {
		log.Errorf("getting system memory: %v", err)
	} else {
	...
	if err != nil {
		log.Errorf("getting cpu info: %v", err)
	} else {
